...
{ lie_Index i,r=g->lierank; entry level=0,result; boolean neg;
...
local matrix* simp_refl(entry* alpha,simpgrp* g,matrix* m, lie_Index offset)
{ lie_Index i,r=g->lierank; entry* row=mkintarray(2*r),* col=&row[r];
  
  { lie_Index norm_alpha=simp_norm(alpha,g); entry* norm=g->root_norm->compon;
...
{ lie_Index i,j,t=0; matrix* R=mat_id(Lierank(grp));
...
{ lie_Index i=0,l=0; lie_Index s=Ssrank(grp); /* do not affect the toral part */
...
local lie_Index dom_length(entry* lambda)
{ lie_Index l,s=Ssrank(grp); entry* x=mkintarray(s);  copyrow(lambda,x,s);
...
{ lie_Index i,s=Ssrank(grp),l=0; entry* x=mkintarray(s),wi;
...
{ lie_Index i=0,j=0,s=Ssrank(grp); entry* x=mkintarray(s);
...
finish: result=Wwordv(y,l); freearr(x);
  ...
  { matrix* check=Weyl_mat(result); lie_Index r=Lierank(grp);
...
{ lie_Index i,l=w->ncomp,s=Ssrank(grp),wi;
...
{ lie_Index k,n=m->nrows,s=Ssrank(grp); matrix* result=mkmatrix(n,m->ncols);
  entry* x=mkintarray(s);
  for (k=0; k<n; ++k)
  { entry* w= m->elm[k]; lie_Index i,l=m->ncols,wi; vector* t;
    for (i=0; i<s; ++i) x[i]=1;
    for (i=l-1; i>=0; --i)
...
{ lie_Index i,j,s=Ssrank(grp),count=0; entry* inx=mkintarray(m->nrows);
  for (i=0; i<m->nrows; ++i) /* get indices of dominant rows into |inx| */
  { for (j=0; j<s; ++j)  if (m->elm[i][j]<0) break;
    if (j==s) inx[count++]=i;
  }
  { lie_Index r=m->ncols; matrix* result=mkmatrix(count,r);
...
{ lie_Index i,j,s=Ssrank(grp),count=0; entry* inx=mkintarray(p->nrows);
  ...
  { lie_Index r=p->ncols; poly* result=mkpoly(count,r);
    for (i=0; i<count; ++i)
...
local entry* fix_vec (vector* I, lie_Index s)
{ lie_Index i; entry* x=mkintarray(s);
  ...
  for (i=0; i<lw; i++)
    ...
    { entry wi=w[i]-1; /* internally, reflections are numbered $0,\ldots,s-1$ */
      if (w[i]==0) --l; /* ignore zero entry */
        ...
        { lie_Index j=i;  copyrow(x,y,s); /* make a temporary copy of~|x| */
          do
            if (w[--j]!=0)
...
{ vector* empty=mkvector(0),* result; lie_Index lw=v->ncomp;
...
{ 
  ...
  { lie_Index i,l=lw; entry* x=fix_vec(R,Ssrank(grp));
...
{ lie_Index i,s=Ssrank(grp), d; entry* delta=mkintarray(s);
  
  { matrix* ic=Icartan(); lie_Index dc=Detcartan(); entry* t=mkintarray(s);
...
{ lie_Index i,s=Ssrank(grp); entry* x=mkintarray(2*s),* y=&x[s]; boolean result;
...
matrix* Bh_desc(entry* w, lie_Index l) /* |w| is reduced of length |l| */
{ entry* inx; lie_Index s=Ssrank(grp),n_desc=0;
  if (l==0) return mkmatrix(0,0); /* avoid attempting a $0\times-1$ matrix */
  
  { lie_Index i,j; entry* x=(inx=mkintarray(l+s),&inx[l]);
    for (i=0; i<l; ++i)
  ...
  { matrix* result=mkmatrix(n_desc,l-1); lie_Index i,j;
    for (i=0; i<n_desc; ++i)
...
{ entry* inx; lie_Index s=Ssrank(grp),n_desc=0;
  ...
  { lie_Index i,j; entry* x=(inx=mkintarray(l+2*s),&inx[l]), *y=&x[s];
    for (i=0; i<l; ++i)
    { for (j=0; j<s; ++j) x[j]=1; /* |x=rho| */
      for (j=0; j<l; ++j)  if (j!=i)
      { entry wj=w[j]-1; if (x[wj]>0) w_refl(x,wj); else break; }
...
local void Bh_descs(matrix** m, entry* lwb, lie_Index s)
{ vector* word=Wwordv((*m)->elm[0],-1); entry* w=word->compon;
  lie_Index i,j,k,l=word->ncomp,n_desc=0;
...
local poly* R(entry* x, entry* y, lie_Index k, lie_Index d, lie_Index s)
  /* modifies |x| and |y| */
{ lie_Index i; entry* v=mkintarray(2*s),* w=&v[s];
...
{ lie_Index i,s=Ssrank(grp); entry* v=mkintarray(2*s),* w=&v[s]; poly* result;
  for (i=0; i<s; ++i) v[i]=w[i]=1;
  Waction(v,x); Waction(w,y);
  result=R(v,w,0,dom_length(w)-dom_length(v),s); freearr(v); return result;
}

local poly* KL(entry* x, entry* y, lie_Index e, lie_Index d, lie_Index s)
{ lie_Index i; entry* v=mkintarray(2*s),* w=&v[s]; poly* result;
  ...
  { matrix* level=mkmatrix(1,s); lie_Index j,c=1; /* |c==(l(y)-l(z)+1)/2| */
