...
{ lie_Index i,r=g->lierank; entry* res=mkintarray(r),* ones=mkintarray(r);
...
local lie_Index locate(entry* mu,entry** m,entry* rho_hat,entry* level,simpgrp* g)
{ lie_Index l=0,i,r=g->lierank; entry* lambda=m[0]; /* the highest weight */
...
{ lie_Index i,j,k,l,r=g->lierank,n=g->roots->nrows;
...
{ if (type_of(grp)==SIMPGRP) return simp_dim_irr(lambda,&grp->s);
  ...
  { lie_Index i,s=0; bigint* res=one;
...
bigint* Dim(poly* p)
{ lie_Index i,r=p->ncols; bigint* res=null; entry* lambda= mkintarray(r);
  for (i=0; i<p->nrows; ++i)
...
{ if (type_of(grp)==SIMPGRP) return simp_domw(lambda->compon,&grp->s,NULL);
  ...
  { lie_Index i,j,n=grp->g.ncomp,r=Lierank(grp);
...
{ lie_Index i=0,j=0; entry* pos,* offset,* lambda; matrix* result;
                         ...
                         { lie_Index i; entry* mu=dom_w->elm[k];
                           ...
                           for (i=0; i<roots->nrows; ++i)
                             ...
                             { entry* alpha=roots->elm[i];
                               ...
                               { lie_Index k; entry* a=g->roots->elm[i]; entry inp=0,d=0;
...
{ lie_Index i,j,r=Lierank(grp),s=Ssrank(grp); poly* result;
...
{ lie_Index i,r=p->ncols; poly* result=poly_null(r);
  for (i=0; i<p->nrows; ++i)
  { testdom(p->elm[i],grp);
...
{ if (type_of(g)==SIMPGRP) return simp_level_vec(&g->s);
  ...
  { lie_Index i,j=0,r=Lierank(g); entry* result=mkintarray(r);
