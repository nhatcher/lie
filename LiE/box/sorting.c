...
local lie_Index split_vec(entry* a,lie_Index n)
...
local void heapify_p(poly* p,lie_Index i,lie_Index n)
{ lie_Index l,len=p->ncols; entry** a=p->elm-1; bigint** coef=p->coef-1;
  while ((l=i<<1)<=n)
  { lie_Index d= (*compare)(a[i],a[l],len)<0 ? i : l;
    if (l<n && (*compare)(a[d],a[l+1],len)>0) d=l+1;
...
{ lie_Index i,n=m->nrows; for (i=n; i>0; i--) heapify_m(m,i,n); }

local void build_heap_p(poly* p)
{ lie_Index i,n=p->nrows; for (i=n; i>0; i--) heapify_p(p,i,n); }
...
{ if (n>=3)
  { lie_Index i=split_vec(a,n); sortrow(a,i); sortrow(&a[i+1],n-i-1); }
