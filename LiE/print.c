...
{ lie_Index i; int coef_width=0;
  ...
  { int col=indent(lmargin), rowsize;
    { int start=col; 
      col+=printbigint(p->coef[0],coef_width); col+=Printf("X");
      col=print_row(p->ncols, p->elm[0],widths,col); rowsize=col-start;
    }
    for (i=1; i<p->nrows; ++i)
    { 
      ...
      { boolean neg=p->coef[i]->size<0;
        if (neg) p->coef[i]->size=-p->coef[i]->size; /* take absolute value */
        col+=Printf(neg ? " -" : " +");
        col+=printbigint(p->coef[i],coef_width);
        if (neg) p->coef[i]->size=-p->coef[i]->size; /* restore sign */
      }
      col+=Printf("X"); col=print_row(p->ncols, p->elm[i],widths,col);
    }
  }
  freearr(widths);
  Printf("\n");
...
local void listvar(symblst topsym, boolean pr)
{
  ...
  if (pr && (isshared(obj)>1)) Printf("\t(shared %ld)",(long)(isshared(obj)-1));
  Printf("\n");
}
